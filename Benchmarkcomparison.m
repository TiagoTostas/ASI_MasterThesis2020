...
title('Positive Predictivity','FontSize',20)
set(gcf,'defaultfigurecolor','w')
ylim([0 1])

%%
Pth_values = 1:0.2:6;
Se_total = mean(Se(:,11)).*100;
P_total = mean(P(:,11)).*100;
Der_total = mean(Der(:,1)).*100;
F1_total = mean(F1(:,11)).*100;


plot(Pth_values,Se_total)
hold on
plot(Pth_values,P_total)

ylabel('%')
xlabel('P_{th} values')
legend('Sensitivity', 'Positive Predictivity','Location','SE')
grid minor
